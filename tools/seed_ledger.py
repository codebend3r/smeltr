...
STATE = [
    ...
    ("Armageddon (1998)",            78.12, 31.23, 3,  5, "Vhagar/A",    "restarted after a reboot killed the first run at ~75%"),
    ("What Dreams May Come (1998)",  75.22, 27.84, 3, 63, "Vermithor/W", ""),
    ("Sudden Death (1995)",          74.64, 24.79, 3,  1, "Vhagar/S",    ""),
    ("Oldboy (Oldeuboi) (2003)",     73.91, 52.71, 1,  5, "Vhagar/O",    "heavy grain; weakest ratio of the job"),
    ("Riddick (2013)",               71.52, 17.92, 1,  1, "Vhagar/R",    ""),
    ("Gemini Man (2019)",            76.35, 18.87, 9, 19, "Vhagar/G",    "59.94 fps HFR; largest single reclaim of the job"),
    ("Leaving Las Vegas (1995)",     70.64, 40.99, 4,  4, "Vhagar/L",    "heavy 90s film grain; 14h51m; 0 decoder errors"),
]
...
for title, src, out, a, s, dest, note in STATE:
    core.record(Entry(
        ...
        audio=a, subs=s, dest=dest, note=note,
        provenance="state-file", exact=False,
    ))
...
core.record(Entry(
    ...
    audio=1, subs=28, dest="Vermithor/W",
    ...
    provenance="recovered", exact=False,
))

rows = core.ledger()
print(f"seeded {len(rows)} rows -> {core.LEDGER}")
known = [r for r in rows if r["saved_bytes"]]
print(f"reclaimed (from {len(known)} rows with both sizes): "
