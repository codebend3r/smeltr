...
function node(tag, cls, text) {
  return {
    ...
    value: undefined, disabled: false, title: '', type: '', draggable: false,
    children: [], attrs: {}, dataset: {}, classList: { add() {}, remove() {} }, on: {},
    appendChild(c) { this.children.push(c); return c; },
    setAttribute(k, v) { this.attrs[k] = v; },
    addEventListener(ev, f) { (this.on[ev] = this.on[ev] || []).push(f); },
    fire(ev) { (this.on[ev] || []).forEach(f => f({ stopPropagation() {} })); },
  };
}
...
console.log('the menu is the ladder');
{
  ...
  check('one option per rung, plus auto',
...
console.log('a hand-picked value reads back');
{
  const sel = ui.crfPicker(row({ crf: 12, crf_set: true }), S);
  check('the chosen rung is selected', sel.value === '12', sel.value);
  check('and the cell says a human chose it', / set$| set /.test(sel.className),
    sel.className);
  check('its tooltip says so too, in words',
    /Chosen by hand/.test(sel.title), sel.title);
}
{
  const sel = ui.crfPicker(row(), S);
...
console.log('what a change actually sends');
{
  posted.length = 0;
  const sel = ui.crfPicker(row(), S);
  sel.value = '12'; sel.fire('change');
  check('picking a rung writes that rung',
    posted.length === 1 && posted[0][0] === '/api/queue/crf' &&
    posted[0][1].crf === 12 && posted[0][1].title === 'Kubo (2016)',
...
}
{
  ...
  check('picking auto CLEARS the override rather than pinning the default',
    posted.length === 1 && posted[0][1].crf === null, JSON.stringify(posted));
}

console.log('an open picker defers the repaint, and releases it');
{
  const sel = ui.crfPicker(row(), S);
  sel.fire('focus');
  check('focus marks the row so paint() skips the rebuild',
    ui.open() === 'Kubo (2016)', String(ui.open()));
  sel.fire('blur');
...
}
{
  const sel = ui.crfPicker(row(), S);
  sel.fire('focus');
  sel.value = '18'; sel.fire('change');
  check('a committed change releases it too — the response repaints, and the '
    + 'deferral must not wedge against a node that is about to be replaced',
    ui.open() === null, String(ui.open()));
}

console.log('one CRF control per row');
{
  const acts = ui.rowActions(row({ ready: true }), S);
  const selects = acts.children.filter(c => c.tagName === 'select');
  check('the hover actions carry no second picker',
...
console.log('a running encode gets no picker');
{
  ...
  const cell = src.slice(src.indexOf('var crfTd;'),
                         src.indexOf('tr.appendChild(crfTd);'));
  check('the encoding branch renders TEXT, never crfPicker',
    /r\.encoding\)\{[\s\S]*?crfTd=el\("td","n q-crf",String\(/.test(cell) &&
    cell.indexOf('crfPicker') > cell.indexOf('else{'),
    cell);
  check('the skipped row still claims no number',
    /r\.skipped\)\{ crfTd=el\("td","n q-crf","—"\)/.test(cell), cell);
}
