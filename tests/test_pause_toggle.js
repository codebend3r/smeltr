 ...
 *
 *   node tests/test_pause_toggle.js
 *
...
function node(tag, cls, text) {
  return {
    ...
    disabled: false, attrs: {}, handlers: [], children: [],
    setAttribute(k, v) { this.attrs[k] = v; },
    getAttribute(k) { return this.attrs[k]; },
    appendChild(c) { this.children.push(c); return c; },
    addEventListener(_ev, h) { this.handlers.push(h); },
    click() { if (this.disabled) return false; this.handlers.forEach(h => h()); return true; },
  };
}
...
function harness() {
  ...
  const api = (path, payload) => {
    ...
    calls.push({ path, payload, settle: () => { settle(); return p; }, p });
    return p;
  };
  ...
  const code = [
    ...
    fn('pauseState'), fn('pauseSend'), fn('pauseSwitch'),
...
(async function () {
  console.log('the harness still mirrors web/app.js');
  check('pauseWant and pauseBusy are module-level vars there',
  ...
  check('renderLive draws through pauseState, not summary.paused directly',
    /var\s+paused\s*=\s*pauseState\(s\);/.test(src),
    'an unsettled click would wait for SMB before it showed');

  console.log('the control is ONE native button -- mouse, finger and keyboard');
  {
    const { mod, calls } = harness();
    const row = mod.pauseSwitch(false, 'pause after this encode');
    const pill = row.children[0], lbl = row.children[1];
    check('the control itself is a <button>', row.tagName === 'button',
    ...
    check('it is the switch a screen reader drives',
      row.getAttribute('role') === 'switch' &&
      row.getAttribute('aria-checked') === 'false');
    check('the sentence is INSIDE it, so aiming at the words works',
      lbl.className.indexOf('swt-label') >= 0 && lbl.textContent.length > 0,
      'only the 36x20 pill used to be clickable');
    check('the pill is decoration inside it, not a second control',
      pill.tagName === 'span' && pill.getAttribute('aria-hidden') === 'true' &&
      pill.handlers.length === 0);
    check('exactly one handler -- two would fire twice and toggle back',
      row.handlers.length === 1);
    row.click();
    check('one write, from a click anywhere on it', calls.length === 1 &&
      calls[0].payload.paused === true);
  }

  console.log('and its CSS makes it a real target on a touch screen');
  {
    check('it hugs pill+sentence rather than the whole card width',
      /\.pauserow\{[^}]*display:inline-flex/.test(css),
      'a full-width target would toggle the pipeline on a stray tap');
    check('it carries the button reset now that it IS the button',
      /\.pauserow\{[^}]*border:0/.test(css) &&
      /\.pauserow\{[^}]*background:none/.test(css));
    check('it says it is clickable', /\.pauserow\{[^}]*cursor:pointer/.test(css));
    check('taps are not delayed by double-tap zoom',
      /\.pauserow\{[^}]*touch-action:manipulation/.test(css));
    check('a finger gets a 44px target',
      /@media \(pointer:coarse\)\{\s*\.pauserow\{min-height:44px\}/.test(css),
      'the pill is only 20px tall');
    check('focus is still visible on the button',
      /\.pauserow:focus-visible\{/.test(css));
    check('the pill no longer claims to be interactive itself',
      !/\.swt\{[^}]*cursor:pointer/.test(css) && !/\.swt:focus-visible/.test(css));
  }

  console.log('a click is never swallowed');
  {
    const { mod, calls } = harness();
    const row = mod.pauseSwitch(false, 'pause after this encode');
    ...
    check('one write went out, asking to pause',
      calls.length === 1 && calls[0].payload.paused === true,
      JSON.stringify(calls.map(c => c.payload)));
    check('the flip is drawn before the server answers -- not after SMB',
      mod.drawn() === true, 'drawn=' + mod.drawn());
    check('but the server value has NOT been assumed',
      mod.server() === false);
  }

  console.log('the last click wins');
  {
    const { mod, calls } = harness();
    ...
    check('no second round-trip races the first', calls.length === 1,
      'saw ' + calls.length + ' concurrent writes');
    check('the newest intent is what the card draws', mod.drawn() === false);
    calls[0].settle(); await tick(); await tick();
    check('the queued intent is sent when the first write lands',
      calls.length === 2, 'saw ' + calls.length + ' writes');
    check('and it carries the LAST click',
      calls.length === 2 && calls[1].payload.paused === false,
      calls.length === 2 ? JSON.stringify(calls[1].payload) : 'no second write');
  }

  console.log('clicking back to where it started asks for nothing');
  {
    const { mod, calls } = harness();
    ...
    mod.setState(true);
    calls[0].settle(); await tick(); await tick();
    check('the settled write already carries the final intent -- no redundant POST',
      calls.length === 1, 'saw ' + calls.length + ' writes for a round trip back');
    check('and the card still draws it', mod.drawn() === true);
  }

  console.log('a settled write hands the switch back to the server');
  {
    const { mod, calls } = harness();
    mod.pauseSwitch(false, '').click();
    ...
    check('and the server value alone now decides',
      mod.pauseState({ paused: false }) === false,
      'a stale optimistic flip outlived its write');
  }

  console.log('a write that never took snaps back to the truth');
  {
    const { mod, calls } = harness();
    mod.pauseSwitch(false, '').click();
    ...
    calls[0].settle(); await tick(); await tick();
    check('the switch returns to the un-paused truth', mod.drawn() === false,
      'a denied LAN write would leave the switch lying');
  }

  console.log('a later click still works after all of that');
  {
    const { mod, calls } = harness();
    for (let i = 0; i < 6; i++) {
      const on = i % 2 === 1;
    ...
    check('six consecutive toggles produced six writes', calls.length === 6,
      'saw ' + calls.length);
