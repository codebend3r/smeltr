...
const arriving = (done, rate): QRow => ({
  ...
  bytes: 67 * GIB,
  location: "Vermithor",
  src_dir: "Media/4K Family Movies/C",
  skipped: false,
  pinned: false,
  encoding: false,
  ready: false,
...
sect("growing bytes must NOT change the key");
check(
  "a queue row pulling faster does not move qShape",
...
check(
  "a push moving does not move xShape",
  key(api.xShape(xfer(1 * GIB))) === key(api.xShape(xfer(30 * GIB))),
);

sect("shape changes MUST change the key");
check(
  "a pull that starts moves qShape",
  (() => {
    const idle = arriving(null, 0);
    idle.arriving_bytes = null;
    return key(api.qShape(idle)) !== key(api.qShape(arriving(1 * GIB, 8e6)));
  })(),
);
check(
  "a pull that stalls moves qShape",
  (() => {
    const s = arriving(9 * GIB, 0);
    s.arriving_stalled = true;
    return key(api.qShape(s)) !== key(api.qShape(arriving(9 * GIB, 8e6)));
  })(),
);
check(
  "a push that stalls moves xShape",
  key(api.xShape(xfer(9 * GIB, { stalled: true }))) !== key(api.xShape(xfer(9 * GIB))),
);
check(
  "skipping moves qShape",
  (() => {
    const r = arriving(1 * GIB, 8e6);
    const s = Object.assign({}, r, { skipped: true });
    return key(api.qShape(r)) !== key(api.qShape(s));
  })(),
);
check(
  "a hand-picked CRF moves qShape",
  (() => {
...
check(
  "pinning the default rung still moves qShape",
  (() => {
...
check(
  "a queue position change moves qShape",
  (() => {
...
sect("progWrite writes in place");
const td = node("td");
...
const fillNode = first.fill,
  txtNode = first.txt;
api.progWrite("arr|croods", api.arrState(arriving(9 * GIB, 8e6)));
check(
  "the same shape reuses the fill node (keeps its width transition)",
  api.refs()["arr|croods"].fill === fillNode && builds === buildsAfterFirst,
);
check(
  "the caption is the same node, rewritten",
  api.refs()["arr|croods"].txt === txtNode && txtNode.textContent.indexOf("9.00 GiB") >= 0,
);
check(
  "the bar width advanced",
  fillNode.style.width === (9 / 67) * 100 + "%",
  "got " + fillNode.style.width,
);
const stalled = arriving(9 * GIB, 0);
stalled.arriving_stalled = true;
api.progWrite("arr|croods", api.arrState(stalled));
check("a shape change DOES rebuild the slot", builds > buildsAfterFirst);
check(
  "a stalled pull draws no bar, ever",
  api.refs()["arr|croods"].fill === null && api.refs()["arr|croods"].mark.textContent === "stalled",
);
check(
  "a stalled pull says it is not moving",
  api.refs()["arr|croods"].txt.textContent.indexOf("not moving") >= 0,
);

sect("captions keep their units and do not repeat the destination");
check(
  "the caption does not repeat the destination (the pills are on the ledger row above)",
  api.xferState(xfer(9 * GIB)).text.indexOf("copied to") < 0 &&
    api.xferState(xfer(9 * GIB)).text.indexOf("copied") >= 0,
);
check(
  "a moving push carries a binary rate and a marked ETA",
  ["MiB/s", "~", "left"].every((w) => api.xferState(xfer(9 * GIB)).text.indexOf(w) >= 0),
);
check(
  "the rate is never decimal MB/s beside binary GiB operands",
  api.xferState(xfer(9 * GIB)).text.indexOf("MB/s") < 0 ||
    api.xferState(xfer(9 * GIB)).text.indexOf("MiB/s") >= 0,
);
check(
  "a stalled push claims no rate and no ETA",
  api.xferState(xfer(9 * GIB, { stalled: true })).text.indexOf("MiB/s") < 0,
);
check(
  "both operands carry GiB",
  (api.xferState(xfer(9 * GIB)).text.match(/GiB/g) || []).length === 2,
);
check(
  "a stalled push is never given a percentage bar",
  api.xferState(xfer(9 * GIB, { stalled: true })).shape === "stall",
);
check(
  "an over-large partial is a stale leftover, not 110% progress",
