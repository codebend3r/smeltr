 ...
 *
 *   node tests/test_sysmon_render.js
 *
 * The other two node suites test the chart's arithmetic. This one tests the
...
const code = [
  ...
  decl(/var MON_STOPS=\[[^\]]*\];/),
  decl(/var MON_GUT=\d+, MON_PADT=\d+, MON_AXIS=\d+;/),
...
function recorder(w, h) {
  ...
  const ctx = {
    ...
    beginPath() { cur = []; },
    moveTo(x, y) { cur.push({ m: 'M', x, y }); },
    lineTo(x, y) { cur.push({ m: 'L', x, y }); },
    stroke() {
      ops.push({ op: 'stroke', pts: cur.slice(), stroke: ctx.strokeStyle, width: ctx.lineWidth });
    },
    fillText(t, x, y) { ops.push({ op: 'text', t, x, y }); },
  };
...
const W = 800, H = 118;
const X0 = api.MON_GUT, X1 = W - 6, Y0 = api.MON_PADT;
const Y1 = H - api.MON_AXIS;            // every chart under test sets axis:true
const COLS = Math.round(X1 - X0);
const CSS = { ink: 'INK', ink3: 'INK3', grid: 'GRID', nodata: 'NODATA',
  nodataBd: 'NODATA-BD', mono: 'mono', histStart: 0, histLabel: '21:45' };

/* Three series on one chart, matching the Utilization card. */
function chart(rec) {
  return {
    canvas: rec.canvas, axis: true, pctAxis: true, scale: 1,
    series: [{ k: 0, tok: '--ch-1' }, { k: 1, tok: '--ch-2' }, { k: 2, tok: '--ch-3' }],
  };
}
...
console.log('full history, every stop');
ring(NOW, SLOTS);
for (const span of STOPS) {
  ...
  const first = d.series[0].pts[0], last = d.series[0].pts[d.series[0].pts.length - 1];
  ...
  ok(d.series[0].pts.length > COLS * 0.9,
...
console.log('');
console.log('gridlines and tick labels');
for (const span of STOPS) {
  const d = draw(NOW, span, NOW - SLOTS + 1);
  const label = api.spanLabel(span);
...
for (const span of STOPS) {
  ...
  near(d.washes[0].w, overhang * (X1 - X0), 2,
  ...
  near(d.edges[0].pts[0].x, X0 + d.washes[0].w, 1,
  ...
  near(s0[1].x, X0 + overhang * COLS, 2,
    `${label}: the baseline ends where the history begins`);
  ok(!s0.slice(1).some(p => p.m === 'M'),
...
console.log('a window narrower than the canvas is wide');
{
  /* 900 seconds of samples on a 1200 px canvas: there are fewer seconds
     ...
     it stays caught. */
  const WIDE = 1240;
  const wx0 = api.MON_GUT, wx1 = WIDE - 6;
...
console.log('');
console.log('a missing second inside the history');
{
  ...
  ok(breaks.length >= 1,
    'a sampler outage breaks the path -- the 0 baseline never bridges it');
...
console.log('sysmon render: all passed');
